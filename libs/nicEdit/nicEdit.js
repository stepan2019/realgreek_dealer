...
var nicEditorInstance = bkClass.extend({
	...
	construct : function(e,options,nicEditor) {
		...
		if(isTextarea || this.options.hasPanel) {
			...
			var s = {width: newX+'px', border : '1px solid #ccc', borderTop : 0, overflowY : 'auto', overflowX: 'hidden',background: '#ffffff' };
			s[(ie7s) ? 'height' : 'maxHeight'] = (this.ne.options.maxHeight) ? this.ne.options.maxHeight+'px' : null;
			this.editorContain = new bkElement('DIV').setStyle(s).appendBefore(e);

// custom !!!!
// fix for error display
			var fname = e.getAttribute('id'); // set id of div field_name_elm
			var editorElm = new bkElement('DIV').setAttributes({'id' : fname+'_elm'}).setStyle({width : (newX-8)+'px', margin: '4px', minHeight : newY+'px'}).addClass('main').appendTo(this.editorContain);
	...
	keyDown : function(e,t) {
		...
		if(this.ne.options.countTextLeft) { this.CountTextLeft();}
		if(e.ctrlKey) {
	...
	CountTextLeft : function () {

	if (typeof document.newad.no_words == 'undefined') return;
...
//alert(text);
	...
	if(no_words>0) {
		...
		document.getElementById(this.ne.options.count_input).value = words_left;
	}
