...
class acategories {
	...
	public function __construct()
	{
		...
		$this->settings_table = $config_table_prefix."ac_settings";
	...
	function getSettings() {
		...
		global $db;
		$ac_settings = $db->fetchAssoc("select * from ".$this->settings_table);

		foreach ($ac_settings as $key=>$value) {
			if($key=='categories_list') continue;
			$row[$key] = clean($value);
		}


		$ac_settings['array_categories_list'] = explode(",", $ac_settings['categories_list']);
		return $ac_settings;
