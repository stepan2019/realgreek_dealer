...
if($_POST) {
	...
	foreach($_POST as $key=>$value) {
		...
		if(isset($_POST['delete_selected']) || isset($_POST['delete_selected_x']))  $blocked_phones->deleteFromWhitelist($id);
	}
	...
	if(isset($_POST['Add'])) {
		...
		$blocked_phones->AddBulkToWhitelist($word, $info);

	}

	if(isset($_POST['delete_selected']) || isset($_POST['delete_selected_x'])) {

		$location="whitelist_phones.php?page=".$page;
...
$array_whitelist_phones = $blocked_phones->searchWhitelistphones($post_array, $start, $no_per_page);
$no_phones = $blocked_phones->getNoWhitelistphones($post_array);

$smarty->assign("array_whitelist_phones",$array_whitelist_phones);
...
$smarty->display('whitelist_phones.html');
