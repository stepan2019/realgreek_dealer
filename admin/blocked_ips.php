...
if($_POST) {
	...
	foreach($_POST as $key=>$value) {
		...
		if(isset($_POST['delete_selected']) || isset($_POST['delete_selected_x']))  $blocked_ips->delete($id);
	}
	...
	if(isset($_POST['Add'])) {
		...
		$blocked_ips->AddBulk($word, $info);

	}

	if(isset($_POST['delete_selected']) || isset($_POST['delete_selected_x'])) {

		$location="blocked_ips.php?page=".$page;
...
$array_blocked_ips = $blocked_ips->searchIPs($post_array, $start, $no_per_page);
$no_ips = $blocked_ips->getNo($post_array);

$smarty->assign("array_blocked_ips",$array_blocked_ips);
