...
if($_POST) {
	...
	foreach($_POST as $key=>$value) {
		...
		if(isset($_POST['delete_selected']) || isset($_POST['delete_selected_x']))  $blocked_emails->delete($id);
	}
	...
	if(isset($_POST['Add'])) {
		...
		$blocked_emails->AddBulk($word, $info);

	}

	if(isset($_POST['delete_selected']) || isset($_POST['delete_selected_x'])) {

		$location="blocked_emails.php?page=".$page;
...
$array_blocked_emails = $blocked_emails->searchEmails($post_array, $start, $no_per_page);
$no_emails = $blocked_emails->getNo($post_array);

$smarty->assign("array_blocked_emails",$array_blocked_emails);
