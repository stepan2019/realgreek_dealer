...
if($post) {
	...
	if(empty($ret['error'])) {
		...
		// send email
		$mail2send=new mails();
		$mail2send->init('', '', $your_email, $your_name);

		$array_subject = array("title"=>$ad_title);

		$crt_usr=0;
		$auth=new auth();
		$logged_in = $auth->loggedIn();
	
		$array_message = array("title"=>$ad_title, "ad_link"=>$details_link, "name"=> $your_name, "email"=>$your_email, "message"=>$user_message, "ip"=>getRemoteIp(), "username" => $logged_in);

		$sent = $mail2send->composeAndSend("report_ad", $array_message, $array_subject);
